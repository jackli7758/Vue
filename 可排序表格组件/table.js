Vue.component('vTable',{
    ...
    render:function (h) {//h代表createElement
        let _this=this;
        let trs=[];//二维数组，由currentColumns和currentData组成
        this.currentData.forEach(function (row) {
            let tds=[];
            _this.currentColumns.forEach(function (cell) {
                tds.push(h('td',row[cell.key]));//cell.key=name/age...
            });                                  //row[cell.key]=row.name/row.age/...
            trs.push(h('tr',tds));
        });
        /*<tr>
            <td>row.name</td>
        ...
        */
        let ths=[];//表头节点数组，有排序功能
        this.currentColumns.forEach(function (col,index) {
            if(col.sortable){//如果要求排序
                ths.push(h('th',[
                    h('span',col.title),
                    //升序
                    h('a',{
